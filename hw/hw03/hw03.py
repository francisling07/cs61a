...
def g_iter(n):
    ...
    if n <=3:
        ...
        # compute n-3 elements in the list 
        for i in range(n-3):
            lst.append(lst[0+i]*3 + lst[1+i]*2 + lst[2+i])
        return lst[n-1]
...
def missing_digits(n):
    ...
    if last - rest%10 <= 1:
        ...
        missing = last - rest%10 -1
    return missing_digits(rest) + missing
...
def move_stack(n, start, end):
    ...
    if n == 1:
       ...
       return
    mid = 6 - start - end
    move_stack(n-1, start, mid)    # recursive steps n th step composed of  2 n-1 steps. Therefore the N th step is 2**n-1 moves.
    print_move(start, end)
