...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, passwd):
        ...
        if len(attempts) >= 3:
                return "Too many incorrect attempts. Attempts: " +str(attempts[:3])
        if not password == passwd:
...
def repeated(t, k):
    ...
    assert k > 1
    "*** YOUR CODE HERE ***"
    last = next(t)  # get the 1st item
    occur_times = 1
    for v in t:   # iterate from the 2nd item
        if v == last:
            occur_times += 1
            if occur_times == k:
                return v
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    ## to make clear of this. How to append use of current function.
    check = withdraw(0, old_pass)
    if type(check) == str:  # if the old_pass is wrong, output an error string
        return check 
