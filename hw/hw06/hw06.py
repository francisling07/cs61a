...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.price > self.balance:
            ...
            self.stock -= 1
            self.balance = 0
            return f"Here is your {self.kind} and ${diff} change."

    def restock(self, add_item):
        self.stock += add_item
        return f"Current {self.kind} stock: {self.stock}"

    def add_funds(self, add_money):
        if self.stock == 0:
            return f"Inventory empty. Restocking required. Here is your ${add_money}."
        else:
...
def is_bst(t):
    ...
    #2 all [is_bst(b) for b in branches(t)]
    #3 bst_max(left) <= label(t)
    #4 bst_min(right) > label(t)
    ...
    if t.is_leaf():
        ...
        return (is_bst(right) and is_bst(left) and
